...
BLOG_PERSONA = '美容皮膚科専門医で、レーザー脱毛機器の選定経験豊富な女医'
TONE_GUIDANCE = '- 医療脱毛（永久脱毛）とエステ脱毛（減毛）の違いを医学的に明確化\n- 機器（アレキサンドライト・ダイオード・ヤグ）別の特性を解説\n- 痛みの程度・麻酔オプションを具体的に\n- 「永久に生えてこない」など誇大表現は避け「永久脱毛 = 米国FDA定義」を厳守'
